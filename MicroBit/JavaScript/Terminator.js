...
// Das LCD Display initialisieren
LCD_i2c.LcdBegin(62)
LCD_i2c.clear()
...
// LCD Zeile löschen
function ClearLine(n :number){
    LCD_i2c.setCursor(n, 0)
    LCD_i2c.printText("                ");
    LCD_i2c.setCursor(n, 0)
}
...
radio.onReceivedString(function (receivedString) {
    ClearLine(0)
    LCD_i2c.printText(receivedString)
    serial.writeLine(receivedString)
})
radio.onReceivedNumber(function (receivedNumber) {
    ClearLine(0)
    LCD_i2c.printText(receivedNumber.toString())
    serial.writeLine(receivedNumber.toString())
})
radio.onReceivedValue(function(str, num)
{
    ClearLine(0)
    LCD_i2c.printText(num.toString() + " " + str)
...
basic.forever(function () {
    ...
    if (Math.abs(newPVal - oldPVal) > 2) {
        ...
        ClearLine(1)
        LCD_i2c.printText("Speed: " + newPVal)
